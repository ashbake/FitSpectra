...
def optfunc(x,pix,flux,N,front,end,nspec,errs):
    ...
    #pull out scalings                                                 
    norm = x[(N+4) + N + 2*nspec:(N+4)+N + 2*nspec]

    #pull out tau's
    tau = x[(N+4)+N+3*nspec:(N+4) + N + 3*nspec]

    #pull out thetas
    beta = x[(N+4)+N+4*nspec:(N+4)+N+4*nspec]

    #pull out betas
    theta = x[(N+4)+N+5*nspec:(N+4)+N+5*nspec]
...
def get_vals(x,pix,flux,N,front,end,nspec,numpt):
   ...
   #pull out scalings                                                
    ...
    #pull out thetas
    theta = x[(N+4)+N+4*nspec:(N+4)+N+4*nspec]

    #pull out betas 
    beta = x[(N+4)+N+5*nspec:(N+4)+N+5*nspec]
...
#    truespec = interpolate.splev(lamb, tck)
    return coeff,knots,dxm,dxb,norm,tau,theta,beta,longfit,tck,fit
